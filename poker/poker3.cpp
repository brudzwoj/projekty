...
private:
        ...
        for (int k = bind + 1; k < bind + 7; k++)
        {
                ...
                else if (action == 4)
                {
                    ...
                    if (betOn)
                    {
                        if(players[4].money - betOn < 0){
                            cout << "you dont have enugh money to bet!!" << endl;
                        }else{
                        pot += betOn;
            ...
            else
            {
                ...
                else
                {
                    if (betOn)
                    {
                        if (betOn-players[k % players_count].money < 0)
                        {
                        pot += betOn;
    ...
    void printWinningHand(int winner)
    {
        ...
        for (int min = 0; min < playersLeft(); min++)
        {

    Card otherHand[5];
    for (int i = 0; i < 3; i++)
        otherHand[i] = tableCards[bestHand[min][i]];

    for (int i = 0; i < 2; i++)
        otherHand[i + 3] = players[min].cards[i];

    qsort(otherHand, 5, sizeof(Card), compareCards);


        cout << "   players' "<< players[min].name << " hand:" << endl;
        cout << "   ___   ___   ___   ___   ___" << endl;
        cout << "  | " << ranks[otherHand[0].rank] << " | | " << ranks[otherHand[1].rank] << " | | " << ranks[otherHand[2].rank] << " | | " << ranks[otherHand[3].rank] << " | | " << ranks[otherHand[4].rank] << " |" << endl;
        cout << "  | " << suits[otherHand[0].suit] << " | | " << suits[otherHand[1].suit]<< " | | " << suits[otherHand[2].suit] << " | | " << suits[otherHand[3].suit] << " | | " << suits[otherHand[4].suit] << " |" << endl;
        cout << "  |___| |___| |___| |___| |___|" << endl;
        cout << endl << endl;
        sleep(3);
        }
